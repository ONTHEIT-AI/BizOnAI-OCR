...
# =============================================================================
# Table Normalization Settings (can be set via benchmark CLI options)
# =============================================================================
# These settings only affect TableTest comparisons
TABLE_NORMALIZE_SPACES = False   # Remove all whitespace when comparing table cells
TABLE_NORMALIZE_NEWLINES = False  # Remove <br/> tags (instead of converting to space)


def set_table_normalization(normalize_spaces: bool = False, normalize_newlines: bool = False):
    """Set table normalization options. Called from benchmark.py based on CLI args."""
    global TABLE_NORMALIZE_SPACES, TABLE_NORMALIZE_NEWLINES
    TABLE_NORMALIZE_SPACES = normalize_spaces
    TABLE_NORMALIZE_NEWLINES = normalize_newlines
...
def normalize_text_for_table(text: str) -> str:
    """
    Normalize text for table cell comparison.

    This function applies additional normalization based on TABLE_NORMALIZE_* settings:
    - TABLE_NORMALIZE_NEWLINES: Remove <br/> tags instead of converting to space
    - TABLE_NORMALIZE_SPACES: Remove all whitespace

    These options help handle:
    - Korean decorative spacing: "정 비 계 획" == "정비계획"
    - Multi-line cells: "회사<br/>상호" == "회사상호"
    """
    if text is None:
        return None

    # Step 1: Handle <br/> tags based on setting
    if TABLE_NORMALIZE_NEWLINES:
        # Remove <br/> completely (join without space)
        text = re.sub(r"<br/?>", "", text)
    else:
        # Default: convert to space
        text = re.sub(r"<br/?>", " ", text)

    # Step 2: Remove HTML tags except table-related
    text = re.sub(r"</?(?!table|tr|td|th|thead|tbody|caption)[a-zA-Z][a-zA-Z0-9]*(?:\s[^>]*)?>", " ", text)

    # Step 3: Handle whitespace based on setting
    if TABLE_NORMALIZE_SPACES:
        # Remove all whitespace
        text = re.sub(r'\s+', '', text)
    else:
        # Default: normalize to single space
        text = re.sub(r"\s+", " ", text)
    ...
    # Step 6: Special character replacements
    replacements = {"'": "'", "\u2018": "'", "\u201a": "'", "\u201c": '"', "\u201d": '"', "\u201e": '"',
                    "\uff3f": "_", "\u2013": "-", "\u2014": "-", "\u2011": "-", "\u2012": "-", "\u2212": "-", "\u00b5": "\u03bc"}
    for fancy_char, ascii_char in replacements.items():
        text = text.replace(fancy_char, ascii_char)

    return text.strip() if TABLE_NORMALIZE_SPACES else text
...
class TableTest(BasePDFTest):
    ...
    def __post_init__(self):
        ...
        # Normalize the search text using table-specific normalization
        self.cell = normalize_text_for_table(self.cell)
        self.up = normalize_text_for_table(self.up)
        self.down = normalize_text_for_table(self.down)
        self.left = normalize_text_for_table(self.left)
        self.right = normalize_text_for_table(self.right)
        self.top_heading = normalize_text_for_table(self.top_heading)
        self.left_heading = normalize_text_for_table(self.left_heading)
        ...
        for table_data in tables_to_check:
            ...
            for i in range(table_array.shape[0]):
                for j in range(table_array.shape[1]):
                    cell_content = normalize_text_for_table(table_array[i, j])
            ...
            for row_idx, col_idx in matches:
                ...
                if self.up and row_idx > 0:
                    up_cell = normalize_text_for_table(table_array[row_idx - 1, col_idx])
                ...
                if self.down and row_idx < table_array.shape[0] - 1:
                    down_cell = normalize_text_for_table(table_array[row_idx + 1, col_idx])
                ...
                if self.left and col_idx > 0:
                    left_cell = normalize_text_for_table(table_array[row_idx, col_idx - 1])
                ...
                if self.right and col_idx < table_array.shape[1] - 1:
                    right_cell = normalize_text_for_table(table_array[row_idx, col_idx + 1])
                ...
                if self.top_heading:
                    ...
                    if col_idx in table_data.col_headers:
                        for _, header_text in table_data.col_headers[col_idx]:
                            header_text = normalize_text_for_table(header_text)
                    ...
                    if not top_heading_found and header_rows:
                        for i in sorted(header_rows):
                            if i < row_idx and table_array[i, col_idx].strip():
                                header_text = normalize_text_for_table(table_array[i, col_idx])
                                similarity = fuzz.ratio(self.top_heading, header_text) / 100.0
                                if similarity > best_similarity:
                                    best_similarity = similarity
                                    best_match = header_text
                    ...
                    if not left_heading_found and header_cols:
                        for j in sorted(header_cols):
                            if j < col_idx and table_array[row_idx, j].strip():
                                header_text = normalize_text_for_table(table_array[row_idx, j])
                                similarity = fuzz.ratio(self.left_heading, header_text) / 100.0
                                if similarity > best_similarity:
                                    best_similarity = similarity
                                    best_match = header_text
...
def load_single_test(data: Union[str, Dict]) -> BasePDFTest:
    ...
    if test_type in {TestType.PRESENT.value, TestType.ABSENT.value}:
        ...
        raise ValidationError("MathTest is not supported in KID-benchmark (requires katex/playwright)")
