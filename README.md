...
BizOnAI-OCR is fine-tuned specifically for Korean documents — contracts, financial forms, medical records, government paperwork, and more. It handles Korean-specific layouts, mixed Korean/English/Chinese text, decorative spacing, vertical tables, and complex forms that trip up general-purpose OCR models.

We also release **KDoc-OCRBench**, the first comprehensive Korean document OCR benchmark with 14,738 test cases — designed to fill the gap in standardized Korean OCR evaluation.

## Benchmark Results

### KDoc-OCRBench (Korean)

Evaluated on [**KDoc-OCRBench**](https://huggingface.co/datasets/ONTHEIT/KDoc-OCRBench) — 14,738 unit-test-style assertions across 804 Korean PDFs in 7 industrial document categories (contracts, financial forms, medical records, logistics, education, government, presentations). Tests check text presence/absence, table cell relationships, and header/footer handling. See the [KDoc-OCRBench section](#kdoc-ocrbench-korean-document-ocr-benchmark) below for details.
...
Evaluated on [**KDoc-OCRBench-V2**](https://huggingface.co/datasets/ONTHEIT/KDoc-OCRBench-V2) — **57,062 manually-reviewed unit tests across 850 Korean public-sector PDFs** (statistics, reports, manuals, notices). V2 is a separate, complementary benchmark with much heavier table coverage (49,127 table cell tests, ~12× v1) and openly-shippable source PDFs (KOGL Type 1). See [kdoc_benchmark_v2/](kdoc_benchmark_v2/) for evaluation instructions.

| Model | Baseline | Header/Footer | Long Text | Table | **Overall** |
|-------|---------:|--------------:|----------:|------:|------------:|
| **BizOnAI-OCR** | 98.0 | 95.3 | **78.3** | **59.5** | **82.8** |
...
> **KDoc-OCRBench-V2** — A larger, manually-reviewed second iteration with 850 public-sector PDFs and 57,062 verified tests (with much heavier table coverage) is available as a separate, complementary benchmark. See [kdoc_benchmark_v2/](kdoc_benchmark_v2/) and the dataset card at [ONTHEIT/KDoc-OCRBench-V2](https://huggingface.co/datasets/ONTHEIT/KDoc-OCRBench-V2).
